...
class AsyncReasoningRunner(ReasoningRunner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.rollout_weight_sync_interval = int(
            async_cfg.get("rollout_weight_sync_interval", 1)
        )
        self.max_completed_rollout_batches = int(
            async_cfg.get(
                "max_completed_rollout_batches",
                max(1, self.max_concurrent_rollout_batches),
            )
        )
        self.refill_before_train = bool(async_cfg.get("refill_before_train", True))
        if self.rollout_weight_sync_interval < 1:
            raise ValueError(
                "runner.async_rl.rollout_weight_sync_interval must be >= 1"
            )
        if self.max_completed_rollout_batches < 1:
            raise ValueError(
                "runner.async_rl.max_completed_rollout_batches must be >= 1"
            )
        if self.rollout_weight_sync_interval - 1 > self.max_head_offpolicyness:
            logging.warning(
                "rollout_weight_sync_interval=%s can produce batches older than "
                "max_head_offpolicyness=%s. Increase max_head_offpolicyness or lower "
                "the sync interval to avoid stale-rollout discards.",
                self.rollout_weight_sync_interval,
        ...
        # Running and completed rollout batches both own their output channels.
        # Allocate enough channels to keep generation in flight while the trainer
        # drains a bounded completed-batch buffer.
        self._rollout_channel_count = (
            self.max_concurrent_rollout_batches
    ...
    def _rollout_weight_sync_due(self) -> bool:
        if self._rollout_weight_version is None:
            return True
        return (
            self.global_steps - self._rollout_weight_version
            >= self.rollout_weight_sync_interval
        )
    ...
    def _prepare_rollout_weights_for_submit(self) -> bool:
        if not self._rollout_weight_sync_due():
            return True
        return self._sync_rollout_weights_if_idle(self.global_steps)
    ...
    def _log_step_metrics(
        ...
        async_metrics = {
            ...
            "async/max_completed_buffer_size": self.max_completed_rollout_batches,
            "async/rollout_weight_version": (
                -1
                if self._rollout_weight_version is None
                else int(self._rollout_weight_version)
            ),
            "async/rollout_weight_sync_interval": self.rollout_weight_sync_interval,
        }
