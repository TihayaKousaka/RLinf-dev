...
class ManiSkillRLTPolicySwitchController:
    ...
    def update(
        ...
        *,
        ...
        return self.export_info(device=device)

    def export_info(self, *, device: torch.device) -> dict[str, torch.Tensor]:
        state = self._state_to(device)
        return {
            "rlt_use_actor": state["rlt_use_actor"][:, None],
            ...
            # Keep legacy rollout/replay fields well-defined without expert takeover.
