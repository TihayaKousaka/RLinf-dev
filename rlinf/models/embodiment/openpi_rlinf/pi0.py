...
class Pi0(model.BaseModel):
    ...
    @property
    def z_dim(self) -> int:
        """Width of the frozen Prefix-FT feature exposed to Stage 2."""
        return int(
            self.rlt_cfg.rlt_embed_dim
            if self._resolved_prefix_pool() == "rlt_token"
            else self.rlt_cfg.rlt_input_dim
        )
