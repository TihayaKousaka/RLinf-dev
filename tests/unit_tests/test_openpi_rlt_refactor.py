...
def test_stage1_policy_trains_only_rl_token_reconstruction(monkeypatch):
    ...
    assert RLTStage1Policy.named_parameters is torch.nn.Module.named_parameters
    assert RLTStage1Policy.parameters is torch.nn.Module.parameters
    assert all(
        name.startswith("rl_token_model.") for name, _ in policy.named_parameters()
    )
